...
Backward compatibility wrapper for start_single_piper.launch.py
Maps to: ros2 launch piper piper_unified.launch.py operation_mode:=teleop
...
This file provided basic launch with default parameters, now replaced by unified launcher.
"""
...
def generate_launch_description():
    ...
    import sys
    print("\n" + "="*60, file=sys.stderr)
    print("WARNING: start_single_piper.launch.py is DEPRECATED", file=sys.stderr)
    print("Please use: ros2 launch piper piper_unified.launch.py operation_mode:=teleop", file=sys.stderr)
    print("="*60 + "\n", file=sys.stderr)
    ...
    args = [
        ...
        DeclareLaunchArgument('use_rosbridge', default_value='false', description='Start rosbridge server for remote access'),
    ]
    
    # Path to the unified launch file
    unified_launch_path = os.path.join(os.path.dirname(__file__), 'piper_unified.launch.py')
    
    # Include unified launch with teleop mode
